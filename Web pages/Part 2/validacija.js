...
function ispisiGresku(divElementPoruke) {
    ...
    return{
        ...
        godina:function(inputElement){
            var uslov = true;
            var farbanje = inputElement;
            inputElement = inputElement.options[inputElement.selectedIndex].value
            if (inputElement.lenght < 9) uslov = false; //Provjera da li unesena godina sadrži 8 brojeva i jedan simbol
            else {
                var prvaGodina = inputElement[0] + inputElement[1] + inputElement[2] + inputElement[3]; //Izdvajamo string za prvu godinu
                var drugaGodina = inputElement[5] + inputElement[6] + inputElement[7] + inputElement[8]; //Izdvajamo string za drugu godinu
                if (inputElement[4] != '/') uslov = false; //Provjera da li je odgovarajući znak
                if (parseInt(drugaGodina) != parseInt((parseInt(prvaGodina) + parseInt(1)))) uslov = false;  //Provjera da li je druga godina veća od prve za 1       
                if (parseInt(inputElement[0] + inputElement[1]) != 20 && parseInt(inputElement[5] + inputElement[6]) != 20) uslov = false; //Provjera da li godine počinju sa 20
            }
            if (uslov == false) {
                farbanje.style.backgroundColor = "orangered";
                godina = false;
            }
            else if (uslov == true) {
                farbanje.style.backgroundColor = "white";
                godina = true;
            }
            ispisiGresku(divElementPoruke);
            return uslov;
